...
class TimeRecord(object):
    ...
    def score_record(self, temporal_score_: list[dict[list]], node_size: int, temporal_idx: int, epoch_interval: int):
        """
    ...
    def record_statement(self, given: dict, prefix: str = ""):
        return (
            ...
            f"{round(given['recall'], 8)} " 
            f"{round(given['f1'], 8)} " 
            f"{round(given['micro_prec'], 8)} " 
            f"{round(given['micro_recall'], 8)} " 
            f"{round(given['micro_f1'], 8)} \n"
        )
