...
class TimeRecord(object):
    ...
    def score_statement(self, single_: dict[str, float], mode:str, prefix: str = ""):
        output =  (
            ...
            f"Val F1:           {round(single_['val_f1'], 4):<8} | \n"

            f"Test Acc:         {round(single_['test_acc'], 4):<8} | "
            f"Test Macro Prec:   {round(single_['precision'], 4):<8} | "
            f"Test Macro Rec:    {round(single_['test_roc_auc'], 4):<8} | "
            f"Test Macro F1:     {round(single_['f1'], 4):<8}\n"
        )
        if mode == 'i':
            output = (
                output +
                f"Test New New Acc:  {round(single_['test_new_new_acc'], 4):<8} | "
                f"Test New New F1:   {round(single_['test_new_new_f1'], 4):<8} | "
                f"Test New New AUC:  {round(single_['test_new_new_auc'], 4):<8} | \n"
                f"Test New Old Acc:  {round(single_['test_new_old_acc'], 4):<8} | "
                f"Test New Old F1:   {round(single_['test_new_old_f1'], 4):<8} | "
    ...
    def record_statement(self, given: dict, mode: str, prefix: str = ""):
        output = (
            ...
            f"{round(given['val_f1'], 8)}"
            f"{round(given['test_acc'], 8)} " 
            f"{round(given['precision'], 8)} "  
            f"{round(given['f1'], 8)} " 
            f"{round(given['test_roc_auc'], 8)} \n"
        )
        if mode == 'i':
            output = (
                output+
                f"{round(given['test_new_new_acc'], 8)}"
                f"{round(given['test_new_new_f1'], 8)}"
                f"{round(given['test_new_new_auc'], 8)}"
                f"{round(given['test_new_old_acc'], 8)}"
                f"{round(given['test_new_old_f1'], 8)}"
