...
def train_val(train_val_data: tuple[tuple[np.ndarray]], model: CAWN, mode, bs, epochs, criterion, optimizer, \
    ...
    for epoch in range(epochs):
        ...
        for k in tqdm(range(num_batch)):
            ...
            with torch.no_grad():
                model.eval()
                pred_score = np.concatenate([src_pred.cpu().detach().numpy(), dst_pred.cpu().detach().numpy()])
