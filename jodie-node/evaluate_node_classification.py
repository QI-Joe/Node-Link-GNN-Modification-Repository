...
print("*** Making node classification predictions by forward pass (no t-batching) ***")
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        # PROJECT USER EMBEDDING FOR CLASSIFICATION
        user_projected_embedding = model.forward(user_embedding_input, None, timediffs=user_timediffs_tensor, features=feature_tensor, select='project')
        
        # NEW: Add classification layer to project embeddings to class scores
        class_scores = model.classify(user_projected_embedding)
        
        # NEW: Calculate classification loss
        true_label = y_true[j]  # Assuming y_true contains the true labels for each interaction
        loss += crossEntropyLoss(class_scores, torch.LongTensor([true_label]).cuda())
        
        # NEW: Evaluate classification performance
        _, predicted_label = torch.max(class_scores, 1)
        if j < test_start_idx:
            validation_predictions.append(predicted_label.item())
            validation_true_labels.append(true_label)
        else:
            test_predictions.append(predicted_label.item())
            test_true_labels.append(true_label)
        
        # UPDATE USER EMBEDDING
        user_embedding_output = model.forward(user_embedding_input, None, timediffs=user_timediffs_tensor, features=feature_tensor, select='user_update')
