...
def load_dynamic_dataset(dataset: str = "mathoverflow", fea_dim: int = 64, *wargs) -> tuple[Temporal_Dataloader, dict]:
    ...
    time_pos = time_encoding(timestamp=edges.time)
    pos = (nodes, time_pos)

    graph = Data(x=x, edge_index=edge_index, edge_attr=time, y=labels, pos = pos, time=keep_tracer)
...
def data_load(dataset: str, dynamic: bool = False, for_test:bool=False, **wargs) -> tuple[Temporal_Dataloader, Union[NodeIdxMatching|dict]]:
    dataset = dataset.lower()
    if dynamic:
        if dataset in DYNAMIC:
            return load_dynamic_dataset(dataset=dataset, **wargs)
