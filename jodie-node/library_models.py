...
class JODIE(nn.Module):
    ...
    def reset_prediction(self, num_src: int, num_dest: int):
        self.num_users = num_src
        self.num_items = num_dest
...
def calculate_state_prediction_loss(model, tbatch_interactionids, user_embeddings_time_series, y_true, loss_function):
    ...
    """
    user state change prediction refers to the task of predicting significant changes in a user's status or behavior within a network.
    1. user will be banned or not
    2. whether student will drop the course or not
    """
